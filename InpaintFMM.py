...
''' find closest quadrant : solves a step of the eikonal equation in order to find closest quadrant using fast marching method '''
def solve_eikonal_equation(y1, x1, y2, x2, height, width, dists, flags):
    # check image frame
...
def find_pixel_gradient(y, x, height, width, vals, flags):
    val = vals[y, x]
...
def compute_outside_dists(height, width, dists, flags, band, radius):
    ...
    while band:
        ...
        for nb_y, nb_x in neighbors:
            ...
            last_dist = min([
                solve_eikonal_equation(nb_y - 1, nb_x, nb_y, nb_x - 1, height, width, dists, flags),
                solve_eikonal_equation(nb_y + 1, nb_x, nb_y, nb_x + 1, height, width, dists, flags),
                solve_eikonal_equation(nb_y - 1, nb_x, nb_y, nb_x + 1, height, width, dists, flags),
                solve_eikonal_equation(nb_y + 1, nb_x, nb_y, nb_x - 1, height, width, dists, flags)
            ])
...
def init_mask_dist(height, width, mask, radius):
...
def inpaint_pixel(y, x, img, height, width, dists, flags, radius):
    ...
    dist_grad_y, dist_grad_x = find_pixel_gradient(y, x, height, width, dists, flags)
...
def inpaint(img, mask, radius=5):
    ...
    dists, flags, band = init_mask_dist(height, width, mask, radius)
